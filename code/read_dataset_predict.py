...
def generator_predict(data_dir, shuffle=False):
    ...
    for sample in it:
        ...
        HG = network_to_hypergraph(network_graph=G_copy,
                                   ...
                                   port_stats=P)
        num_samples += 1
        yield hypergraph_to_input_data_predict(HG)
                ...
                for f_id in range(len(T[src, dst]['Flows'])):
                    if T[src, dst]['Flows'][f_id]['AvgBw'] != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                        D_G.add_node('p_{}_{}_{}'.format(src, dst, f_id),
                                     ...
                                     EqLambda = T[src, dst]['Flows'][f_id]['TimeDistParams']['EqLambda'],
                                     AvgPktSize = T[src, dst]['Flows'][f_id]['SizeDistParams']['AvgPktSize'],
...
def hypergraph_to_input_data_predict(hypergraph):
    n_p = 0
    n_l = 0
    mapping = {}
    ...
    return {"traffic": list(nx.get_node_attributes(D_G, 'traffic').values()),
            ...
            "EqLambda": list(nx.get_node_attributes(D_G, 'EqLambda').values()),
            "AvgPktSize": list(nx.get_node_attributes(D_G, 'AvgPktSize').values()),
            "capacity": list(nx.get_node_attributes(D_G, 'capacity').values()),
            "queueSizes": list(nx.get_node_attributes(D_G, 'queueSizes').values()),
    ...
    ds = tf.data.Dataset.from_generator(generator_predict,
                                        ...
                                        output_types=(
                                            {"traffic": tf.float32,
                                             "packets": tf.float32,
                                             "EqLambda": tf.float32,
                                             "AvgPktSize": tf.float32,
                                             "capacity": tf.float32,
                                             "queueSizes": tf.float32,
                                        ...
                                        output_shapes=(
                                            {"traffic": tf.TensorShape([None]),
                                             ...
                                             "queueSizes": tf.TensorShape([None]),
